...
export async function crearComercio({ id, sesion, nombre, distrito, telefono, cuentaPublica, codigoCorto }) {
  const sql = db();
  const [fila] = await sql`
    INSERT INTO comercios (id, sesion_id, nombre, distrito, telefono, cuenta_publica, codigo_corto)
