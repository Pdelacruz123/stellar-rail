...
export const ESQUEMA = [
  ...
  // El codigo de respaldo de 6 numeros de cada cobro: quien no puede escanear
  // el QR lo escribe. Vive lo mismo que el cobro y apunta a su token firmado.
  `CREATE TABLE IF NOT EXISTS cobros_codigos (
     sesion_id  TEXT NOT NULL REFERENCES sesiones(id) ON DELETE CASCADE,
...
/**
 * Guarda el codigo de respaldo de un cobro. Si ese codigo ya esta en uso por
 * otro cobro vigente del mismo espacio, devuelve false y se prueba con otro;
 * uno vencido se reutiliza.
 */
export async function guardarCodigoCobro(sesion, codigo, token, expiraEn) {
  const sql = db();
  const filas = await sql`
    INSERT INTO cobros_codigos (sesion_id, codigo, token, expira_en)
    VALUES (${sesion}, ${codigo}, ${token}, ${expiraEn})
    ON CONFLICT (sesion_id, codigo) DO UPDATE
      SET token = EXCLUDED.token, expira_en = EXCLUDED.expira_en
      WHERE cobros_codigos.expira_en < now()
    RETURNING codigo`;
  return filas.length > 0;
}
...
/** El token del cobro al que apunta un codigo de respaldo vigente, o null. */
export async function tokenDeCodigo(sesion, codigo) {
  const sql = db();
  const [fila] = await sql`
    SELECT token FROM cobros_codigos
     WHERE sesion_id = ${sesion} AND codigo = ${codigo} AND expira_en > now()`;
