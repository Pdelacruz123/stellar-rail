...
import {
  claveAdminValida, codigoCorto, derivarCuenta, nuevaSesion, publicaDe,
} from './cuentas.js';
...
test('la clave del panel se compara bien', () => {
  assert.equal(claveAdminValida('secreta', 'secreta'), true);
  assert.equal(claveAdminValida('otra', 'secreta'), false);
  assert.equal(claveAdminValida('', 'secreta'), false);
  assert.equal(claveAdminValida(undefined, 'secreta'), false);
  assert.throws(() => claveAdminValida('x', undefined), /ADMIN_PASSWORD/);
});
