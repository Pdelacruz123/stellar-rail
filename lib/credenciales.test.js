...
const {
  cifrar, verificar, pinValido, pinAlAzar, normalizarCelular, normalizarCorreo,
  numeroDeTarjeta, normalizarTarjeta, contrasenaAlAzar, contrasenaValida,
...
test('PIN: 4 numeros, sin los que se adivinan al primer intento', () => {
  ...
  for (let i = 0; i < 200; i += 1) assert.equal(pinValido(pinAlAzar()), true);
});
...
test('la contrasena de la demostracion cumple la regla', () => {
  assert.equal(contrasenaValida(contrasenaAlAzar()), true);
  assert.equal(contrasenaValida('corta'), false);
});

test('tarjeta: la tienda puede escribirla como sea', () => {
  const n = numeroDeTarjeta();
  assert.match(n, /^SR-[2-9A-HJKMNP-Z]{4}-[2-9A-HJKMNP-Z]{4}$/);
  assert.equal(normalizarTarjeta(n.toLowerCase().replaceAll('-', ' ')), n);
  assert.equal(normalizarTarjeta('SR-123'), null);
});
