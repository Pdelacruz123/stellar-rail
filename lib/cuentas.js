...
/**
 * Compara la contraseña del panel del emisor sin filtrar informacion por el
 * tiempo que tarda.
 *
 * La URL es publica: sin puerta, cualquiera podria aprobar beneficiarios o
 * vencer programas. La contraseña va en el README para que el jurado entre;
 * es testnet y no protege nada de valor, pero evita que un rastreador o un
 * curioso deje la demo inservible.
 *
 * @param {string} recibida
 * @param {string} [esperada]
 * @returns {boolean}
 */
export function claveAdminValida(recibida, esperada = process.env.ADMIN_PASSWORD) {
  if (!esperada) {
    throw new Error('Falta ADMIN_PASSWORD: el panel del emisor quedaria sin puerta.');
  }
  const a = Buffer.from(String(recibida ?? ''), 'utf8');
  const b = Buffer.from(esperada, 'utf8');
  // timingSafeEqual exige la misma longitud, y la longitud en si ya filtra
  // algo. Se comparan resumenes, que siempre miden lo mismo.
  const ra = createHmac('sha256', 'cmp').update(a).digest();
  const rb = createHmac('sha256', 'cmp').update(b).digest();
