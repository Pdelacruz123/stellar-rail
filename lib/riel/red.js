...
export function crearEnviador({ servidor, networkPassphrase, candado, cuentaSerializada }) {
   ...
   * @param {import('@stellar/stellar-sdk').Memo} [envio.memo]  Queda en el libro publico.
   * @returns {Promise<Resultado>}
   */
  return async function enviar({ fuente, firmantes, operaciones, etiqueta, memo }) {
    ...
    return ejecutar(async () => {
      ...
      for (let intento = 1; intento <= REINTENTOS; intento += 1) {
        ...
        if (memo) constructor.addMemo(memo);
