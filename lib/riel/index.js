...
export function crearRiel(config = {}) {
  ...
  return {
    ...
    /**
     * Descongela varias cuentas en una sola transaccion. Lo usa la entrega
     * de un programa nuevo: las cuentas quedaron congeladas al vencer el
     * anterior, y sin esto la red rechazaria cada entrega.
     *
     * @param {(string|Keypair)[]} cuentas
     */
    descongelarVarias(cuentas) {
      if (cuentas.length === 0) throw new TypeError('No hay cuentas que descongelar.');
      if (cuentas.length > MAX_OPERACIONES) {
        throw new RangeError(
          `Son ${cuentas.length} cuentas y una transaccion admite ${MAX_OPERACIONES} operaciones. `
          + 'Hay que partir el lote.',
      ...
      return desdeElEmisor(operaciones, `descongelarVarias (${cuentas.length} cuentas)`);
    },
